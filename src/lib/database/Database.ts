...
export class Database {
  ...
  constructor(dbName: string) {
    // dbName becomes the filename inside the OPFS root directory
    this.dbName = dbName
  }
  ...
  async open(): Promise<void> {
    ...
    // Register the OPFS VFS. Requires cross-origin isolation — your server must
    // send these headers on the page:
  ...
  async run(sql: string, params: SQLiteCompatibleType[] = []): Promise<RunResult> {
    this.assertOpen()
    let changes = 0
  ...
  async get<T = any>(sql: string, params: SQLiteCompatibleType[] = []): Promise<T | undefined> {
    this.assertOpen()
  ...
  async all<T = any>(sql: string, params: SQLiteCompatibleType[] = []): Promise<T[]> {
    this.assertOpen()
    const rows: T[] = []
